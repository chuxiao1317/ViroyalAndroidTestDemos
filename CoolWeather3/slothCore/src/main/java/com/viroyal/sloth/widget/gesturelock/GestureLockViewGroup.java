package com.viroyal.sloth.widget.gesturelock;
...
import com.viroyal.sloth.widget.gesturelock.GestureLockView.Mode;
...
//		 setMeasuredDimension(mWidth, mHeight);
		...
		if (mGestureLockViews == null)
		{
			...
			if (isScreenChange()){
				mGestureLockViewWidth = (int) (4 * mWidth * 0.7f / (5 * mCount + 1));
			}else {
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		switch (action)
		{
			...
			case MotionEvent.ACTION_UP:
				...
				{
					boolean status = checkAnswer();
					mOnGestureLockViewListener.onGestureEvent(status);
					if (this.mTryTimes == 0)
					{
	...
	public void setmShowLine(boolean mShowLine) {
		this.mShowLine = mShowLine;
	}
